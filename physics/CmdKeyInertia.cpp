...
float RoR::CmdKeyInertia::CalculateCmdOutput(float time, Ogre::SimpleSpline* spline)
{
    ...
    if (spline)
    {
        irr::core::vector3df output = spline->interpolate(time);
...
Ogre::SimpleSpline* RoR::CmdKeyInertiaConfig::GetSplineByName(std::string model)
{
...
void RoR::CmdKeyInertiaConfig::LoadDefaultInertiaModels()
{
    try
    {
        ...
        while (!ds->eof())
        {
            ...
            std::stringUtil::trim(line);

            if (line.empty() || line[0] == ';')
                continue;

            std::stringVector args = std::stringUtil::split(line, ",");
            if (args.size() == 1)
            {
                current_model = line;
            }
            else if (args.size() == 2 && !current_model.empty())
            {
                ...
                // parse the data
                const float point_x = std::stringConverter::parseReal(args[0]);
                const float point_y = std::stringConverter::parseReal(args[1]);

                // attach the points to the spline
                m_splines[current_model].addPoint(irr::core::vector3df(point_x, point_y, 0.0f));
            }
        }
    }
