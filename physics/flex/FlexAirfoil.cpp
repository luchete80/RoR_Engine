...
FlexAirfoil::FlexAirfoil(std::string const & name, Actor* actor, int pnfld, int pnfrd, int pnflu, int pnfru, int pnbld, int pnbrd, int pnblu, int pnbru, std::string const & texband, Vector2 texlf, Vector2 texrf, Vector2 texlb, Vector2 texrb, char mtype, float controlratio, float mind, float maxd, std::string const & afname, float lift_coef, bool break_able)
{
    liftcoef=lift_coef;
...
void FlexAirfoil::updateVerticesPhysics()
{
    ...
    if (breakable) {broken=broken || (vx.crossProduct(vzl).getLengthSQ()>sref)||(vx.crossProduct(vzr).getLengthSQ()>sref);}
    else {broken=(vx.crossProduct(vzl).getLengthSQ()>sref)||(vx.crossProduct(vzr).getLengthSQ()>sref);}
