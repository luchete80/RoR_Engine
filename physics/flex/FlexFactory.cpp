    ...
    std::string resource_group_name)
{
    ...
    m_rig_spawner->SetupNewEntity(entity, irr::video::SColor(0.5, 0.5, 1));
    ...
    std::string const & tire_material_name)
{
    ...
    m_rig_spawner->SetupNewEntity(rim_prop_entity, irr::video::SColor(0, 0.5, 0.8));
    ...
    m_rig_spawner->SetupNewEntity(tire_entity, irr::video::SColor(0, 0.5, 0.8));
...
{
    FLEX_DEBUG_LOG(__FUNCTION__);
    this->WriteToFile((void*)flexbody->m_src_normals, sizeof(irr::core::vector3df) * flexbody->m_vertex_count);
}
...
void FlexBodyFileIO::ReadFlexbodyNormalsBuffer(FlexBodyCacheData* data)
{
    ...
    data->src_normals=(irr::core::vector3df*)malloc(sizeof(irr::core::vector3df) * vertex_count);
    // Read
    this->ReadFromFile((void*)data->src_normals, sizeof(irr::core::vector3df) * vertex_count);
}

void FlexBodyFileIO::WriteFlexbodyPositionsBuffer(FlexBody* flexbody)
{
    FLEX_DEBUG_LOG(__FUNCTION__);
    this->WriteToFile((void*)flexbody->m_dst_pos, sizeof(irr::core::vector3df) * flexbody->m_vertex_count);
}
...
void FlexBodyFileIO::ReadFlexbodyPositionsBuffer(FlexBodyCacheData* data)
{
    ...
    const int vertex_count = data->header.vertex_count;
    // Alloc. Use malloc() because that's how flexbodies were implemented.
    data->dst_pos=(irr::core::vector3df*)malloc(sizeof(irr::core::vector3df) * vertex_count);
    // Read
    this->ReadFromFile((void*)data->dst_pos, sizeof(irr::core::vector3df) * vertex_count);
}
...
void FlexFactory::ResolveFlexbodyLOD(std::string meshname, Ogre::MeshPtr newmesh)
{
    ...
    std::stringUtil::splitBaseFilename(meshname, basename, ext);
    for (int i=0; i<4;i++)
    {
